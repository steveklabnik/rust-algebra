...
const ELEM: Add<f64> = Add(f64::consts::PI);
...
fn app(bencher:&mut test::Bencher) {
    ...
    let a:  Add<f64> =  Add(Arbitrary::arbitrary(gen));
    let b: &Add<f64> = &Add(Arbitrary::arbitrary(gen));
    let task = || {
        a.app(b)
    };
    bencher.iter(task);
...
fn cat_naive(bencher:&mut test::Bencher) {
    ...
    let mut it = xs.iter().map(|&x| Add(x));
    let task = || {
