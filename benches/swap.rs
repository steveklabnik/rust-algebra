...
const ELEM: Swap<Add<f64>> = Swap(Add(f64::consts::PI));
...
fn app(bencher:&mut test::Bencher) {
    ...
    let a:  Swap<Add<f64>> =  Swap(Add(Arbitrary::arbitrary(gen)));
    let b: &Swap<Add<f64>> = &Swap(Add(Arbitrary::arbitrary(gen)));
    let task = || {
        a.app(b)
    };
    bencher.iter(task);
...
fn cat_naive(bencher:&mut test::Bencher) {
    ...
    let mut it = xs.iter().map(|&x| Swap(Add(x)));
    let task = || {
